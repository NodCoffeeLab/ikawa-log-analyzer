...
st.markdown("**(v.0.6 - Final Style Adjustments)**") # 버전 업데이트
...
if st.session_state.processed_logs:
    ...
    with graph_col:
        ...
        for name in selected_profiles_data:
            ...
            if df is not None and color is not None:
                ...
                if TIME_COL in df.columns and INLET_TEMP_COL in df.columns:
                     ...
                     if len(valid_df_inlet) > 1: fig.add_trace(go.Scatter(x=valid_df_inlet[TIME_COL], y=valid_df_inlet[INLET_TEMP_COL], mode='lines', name=f'{name} Inlet Temp', line=dict(color=color, dash='solid'), legendgroup=name), row=1, col=1, secondary_y=False) # 실선으로 변경
                if TIME_COL in df.columns and EXHAUST_ROR_COL in df.columns:
                    ...
                    if len(valid_df_ror) > 1:
                        ...
                        if not ror_df.empty: fig.add_trace(go.Scatter(x=ror_df[TIME_COL], y=ror_df[EXHAUST_ROR_COL], mode='lines', name=f'{name} ROR', line=dict(color=color, dash='dot'), legendgroup=name, showlegend=False), row=1, col=1, secondary_y=True) # 점선 유지
                ...
                if TIME_COL in df.columns and HUMIDITY_COL in df.columns:
                     ...
                     if len(valid_df_hum) > 1:
                         fig.add_trace(go.Scatter(x=valid_df_hum[TIME_COL], y=valid_df_hum[HUMIDITY_COL], mode='lines', name=f'{name} Humidity', line=dict(color=color, dash='solid'), legendgroup=name, showlegend=False), row=2, col=1, secondary_y=False) # 실선, Primary Y축
                ...
                if TIME_COL in df.columns and HUMIDITY_ROC_COL in df.columns:
                     ...
                     if len(valid_df_hum_roc) > 1:
                         fig.add_trace(go.Scatter(x=valid_df_hum_roc[TIME_COL], y=valid_df_hum_roc[HUMIDITY_ROC_COL], mode='lines', name=f'{name} Humidity RoC', line=dict(color=color, dash='solid'), legendgroup=name, showlegend=False), row=2, col=1, secondary_y=True) # 실선, Secondary Y축
                         humidity_plotted_row2 = True
                ...
                if TIME_COL in df.columns and FAN_SPEED_COL in df.columns:
                    ...
                    if len(valid_df_fan) > 1:
                        if valid_df_fan[FAN_SPEED_COL].max() > FAN_SCALE_THRESHOLD:
                            fig.add_trace(go.Scatter(x=valid_df_fan[TIME_COL], y=valid_df_fan[FAN_SPEED_COL], mode='lines', name=f'{name} Fan Speed (High)', line=dict(color=color, dash='solid'), legendgroup=name, showlegend=False), row=3, col=1, secondary_y=False) # 실선, Primary Y축
                        else:
                            fig.add_trace(go.Scatter(x=valid_df_fan[TIME_COL], y=valid_df_fan[FAN_SPEED_COL], mode='lines', name=f'{name} Fan Speed (Low)', line=dict(color=color, dash='solid'), legendgroup=name, showlegend=False), row=3, col=1, secondary_y=True) # 실선, Secondary Y축
        ...
        axis_ranges = st.session_state.axis_ranges
        fig.update_layout(height=1000, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)) # 높이 1000으로 수정

        # X축 업데이트 (상위 2개 눈금 숨김)
        fig.update_xaxes(range=axis_ranges['x'], showticklabels=False, dtick=60, row=1, col=1)
        fig.update_xaxes(range=axis_ranges['x'], showticklabels=False, dtick=60, row=2, col=1) # 습도 X축 눈금 숨김
        fig.update_xaxes(range=axis_ranges['x'], title_text='시간 (초)', dtick=60, row=3, col=1) # 팬 X축만 표시
