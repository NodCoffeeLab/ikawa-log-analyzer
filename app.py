...
st.markdown("**(v.0.7 - Individual Legend Toggle)**") # 버전 업데이트
            ...
            if df is not None and color is not None:
                ...
                if TIME_COL in df.columns and HUMIDITY_COL in df.columns:
                     ...
                     if len(valid_df_hum) > 1:
                         # legendgroup 제거
                         fig.add_trace(go.Scatter(x=valid_df_hum[TIME_COL], y=valid_df_hum[HUMIDITY_COL], mode='lines', name=f'{name} Humidity', line=dict(color=color, dash='solid'), showlegend=True), row=2, col=1, secondary_y=False) # 범례 표시
                ...
                if TIME_COL in df.columns and HUMIDITY_ROC_COL in df.columns:
                     ...
                     if len(valid_df_hum_roc) > 1:
                         # legendgroup 제거
                         fig.add_trace(go.Scatter(x=valid_df_hum_roc[TIME_COL], y=valid_df_hum_roc[HUMIDITY_ROC_COL], mode='lines', name=f'{name} Humidity RoC', line=dict(color=color, dash='solid'), showlegend=True), row=2, col=1, secondary_y=True) # 범례 표시
                ...
                if TIME_COL in df.columns and FAN_SPEED_COL in df.columns:
                    ...
                    if len(valid_df_fan) > 1:
                        if valid_df_fan[FAN_SPEED_COL].max() > FAN_SCALE_THRESHOLD:
                            # legendgroup 제거
                            fig.add_trace(go.Scatter(x=valid_df_fan[TIME_COL], y=valid_df_fan[FAN_SPEED_COL], mode='lines', name=f'{name} Fan Speed (High)', line=dict(color=color, dash='solid'), showlegend=True), row=3, col=1, secondary_y=False) # 범례 표시
                        else:
                            # legendgroup 제거
                            fig.add_trace(go.Scatter(x=valid_df_fan[TIME_COL], y=valid_df_fan[FAN_SPEED_COL], mode='lines', name=f'{name} Fan Speed (Low)', line=dict(color=color, dash='solid'), showlegend=True), row=3, col=1, secondary_y=True) # 범례 표시
